...
import { ErrorText, MainWindow } from "../../styled-components";
...
export const ShowEvent = () => {
  ...
  const displayEventType = () => {
    if (event.category === "Personal Training") return "PT session"
      ...
      <Container m="50px 80px">
        ...
        {errorMsg && (
          <ReusableAlert
            open={!!errorMsg}
            text={errorMsg}
            btnFunction={() => setErrorMsg("")}
          />
        )}
        {event && (
          <ShowEventContent>
            <Heading phone="2rem" desktop="2.7rem" m="15px 0">
              {event.name}
            </Heading>
            ...
            {formatDates.isFinished ? (
              <GreyText>This {displayEventType} has already ended.</GreyText>
            ) : (
              <DateDisplay formatDates={formatDates} />
            )}
            {event.eventImage ? (
              <EventImage src={event.eventImage} alt={event.name} />
            ) : (
              <GreyText>- no image available -</GreyText>
            )}
            <DescriptionDiv>
              ...
              {!formatDates.isFinished && (
                <>
                  {event?.spotsAvailable === 0 && (
                    <ErrorText mt="20px">
                      There are no more spots available for this {displayEventType()}
                    </ErrorText>
                  )}
                  {userIsRegistered && (
                    <>
                      <p>You have registered for this {displayEventType()}</p>
                      <BasicButton
                  ...
                  {event.category !== "Competition" &&
                    ...
                    !userIsRegistered && (
                      <>
                        <SpotsLeft spotsAvailable={event.spotsAvailable}>
                          {event.spotsAvailable}{" "}
                          {event.spotsAvailable === 1 ? "spot" : "spots"} left
                        </SpotsLeft>
                  ...
                  {event.category === "Competition" && !userIsRegistered && (
                    <BasicButton
                      text="Register"
                      color="success"
                      size="large"
