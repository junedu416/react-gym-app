...
export const sortFromMostRecentToOldest = (list, dateAttribute) => {
  const sortedList = list.sort((prev, next) =>
    isMoreRecent(prev[dateAttribute], next[dateAttribute])
  );
  return sortedList;
};
...
export const sortFromOldestToMostRecent = (list, dateAttribute) => {
  const sortedList = list.sort((prev, next) =>
    isMoreRecent(next[dateAttribute], prev[dateAttribute])
  );
  return sortedList;
};
...
export const filterToCurrent = (list, startTime, endTime) => {
  const filteredList = list.filter(
    (item) =>
      moment().isSame(item[startTime], "day") ||
      (moment().isAfter(item[startTime]) && !moment().isAfter(item[endTime]))
  );
  return filteredList;
};
